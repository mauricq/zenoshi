 ...
 * @package App\Service
 */
class FileService implements IServiceProviderInterface
{
    /**
     * @var FileRepository
     */
     ...
     */
    public function __construct(FileRepository $repository, array $checkDuplicated, PrepareDataUtil $prepareDataUtil, string $dateTimeFormat, ServiceProvider $serviceProvider)
    {
    ...
    public function saveGeneric(EntityProvider $object, string $id = null): ?array
    {
        ...
        if ($update) {
            ...
            $object->setIdReward($oldData->getIdFile());
        } else {
            $object->setCreationDate(date_create());
        }
     ...
     * @return File
     */
    public function filter(array $value): File
    {
    ...
    public function getIds(): array
    {
        ...
        return [
            "person_upload_file" => ["id_person_upload_file", "idPersonUploadFile", "Person", "idPerson", "id_person"],
            "merchant" => ["id_merchant", "idMerchant", "Merchant", "idMerchant", "id_merchant"],
            "file_approbation" => ["id_file_approbation", "idFileApprobation", "File", "idFile", "file_location"]
        ];
    }
}